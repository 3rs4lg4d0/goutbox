...
func New(txKey gtbx.TxKey, pool *pgxpool.Pool) *Repository {
	if txKey == nil {
...
func (r *Repository) Save(ctx context.Context, o *gtbx.Outbox) error {
	...
	if !ok {
		return errors.New("a pgx transaction was expected")
	}
...
// the function return the assigned subscription to the caller.
...
func (r *Repository) UpdateSubscription(dispatcherId uuid.UUID) error {
	ctx := context.Background()
